...
func TestSubParserHubIsSubHasChinese(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "10", args: args{filePath: filepath.Join(testRootDir, "[zmk.pw]doctor.who.2005.S01E02.pdtv.xvid.eng.srt")}, want: false},
	}

	subParserHub := NewSubParserHub(logger.GetLogger(), ass.NewParser(logger.GetLogger()), srt.NewParser(logger.GetLogger()))

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {

			bFind, subParserFileInfo, err := subParserHub.DetermineFileTypeFromFile(tt.args.filePath, 0)
			if err != nil {
