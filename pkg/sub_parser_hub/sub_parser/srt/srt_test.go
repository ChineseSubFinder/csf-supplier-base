...
func TestParser_DetermineFileType(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "5", args: args{filePath: filepath.Join(testRootDir, "Criminal.Minds.S13E01.HDTV.x264-KILLERS.en.srt")}, wantNil: false, wantErr: false, wantLang: language.English, wantDialogueLen: 686},
		{name: "6", args: args{filePath: filepath.Join(testRootDir, "Criminal.Minds.S13E01.HDTV.x264-KILLERS.big5.srt")}, wantNil: false, wantErr: false, wantLang: language.ChineseTraditional, wantDialogueLen: 674},
		{name: "7", args: args{filePath: filepath.Join(testRootDir, "Criminal.Minds.S13E01.HDTV.x264-KILLERS.gb.srt")}, wantNil: false, wantErr: false, wantLang: language.ChineseSimple, wantDialogueLen: 674},
		{name: "8", args: args{filePath: filepath.Join(testRootDir, "Criminal.Minds.S03E01.Doubt.DVDRip.XviD-SAiNTS.en.srt")}, wantNil: false, wantErr: false, wantLang: language.English, wantDialogueLen: 769},
		// 特殊一点的字幕
		// 这一个不确定是什么类型的字幕
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			dialogueCount := NewParser(logger.GetLogger()).parseContent(inBytes)
