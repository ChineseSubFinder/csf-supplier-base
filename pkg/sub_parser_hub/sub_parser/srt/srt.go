...
func (p Parser) DetermineFileTypeFromBytes(inBytes []byte) (bool, *subparser.FileInfo, error) {
	...
	subFileInfo.Ext = common.SubExtSRT
...
func (p Parser) parseContent(inBytes []byte) []subparser.OneDialogue {
	...
	*/
	...
	for i, line := range filterLines {
		...
		if steps == 1 {
			// 匹配时间
			// 这里可能有一个梗 00:00:07,180 --> 00:00:10,968  X1:098 X2:614 Y1:444 Y2:527
			// 那么需要优先去除后面的 X1:098 X2:614 Y1:444 Y2:527，然后再执行后续的逻辑
			// newText := re.ReplaceAllString(text, "")
			line = regex_things.ReplaceDialoguePositionInfoSRT.ReplaceAllString(line, "")
			// 剔除两端的空格
